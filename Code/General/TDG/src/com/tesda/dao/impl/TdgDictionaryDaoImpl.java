		...
		if(url.contains("#")){
			...
			for(int i=0;i<strSplitUrls.length;i++){
				...
				if(lstResult.size() != i+1){
					...
					dataConConnectionsDO.setSid(strSplitUrls[i].replaceAll(AppConstant.ORA_URL+dataConConnectionsDO.getHostName()+":"+dataConConnectionsDO.getPortNo()+":", "").trim());
					...
					else if(strSplitUrls[i].contains("sqlServer")){
						...
						dataConConnectionsDO.setPortNo(Long.parseLong(strSplitUrls[i].replaceAll(AppConstant.SQL_SERVER_URL+dataConConnectionsDO.getHostName()+":","").substring(0,strSplitUrls[i].replaceAll(AppConstant.SQL_SERVER_URL+dataConConnectionsDO.getHostName()+":","").indexOf(":"))));
