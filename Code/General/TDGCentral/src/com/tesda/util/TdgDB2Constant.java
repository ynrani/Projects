...
public class TdgDB2Constant extends TdgCentralConstant{
	...
	public static final String ORACLE_GET_ALL_COLUMNS_BY_TAB = "select COLUMN_NAME,TABLE_NAME from USER_TAB_COLUMNS WHERE TABLE_NAME in ({0})";
	public static final String DB2_GET_PK_COLUMN_TYPE = "select cols.column_name,cols.data_type,DATA_PRECISION,CHAR_LENGTH from USER_TAB_COLUMNS cols,USER_CONS_COLUMNS where cols.COLUMN_NAME= USER_CONS_COLUMNS.COLUMN_NAME and USER_CONS_COLUMNS.CONSTRAINT_NAME='{0}' AND cols.TABLE_NAME='{1}'";
	public static final String DB2_GET_ALL_CONSTRAINTS_COLUMNS = "select table_name,column_name,data_type,DATA_PRECISION,CHAR_LENGTH from USER_TAB_COLUMNS where COLUMN_NAME in (select column_name from USER_CONS_COLUMNS where constraint_name in (select constraint_name From user_constraints where CONSTRAINT_TYPE in ('R','U','C')))";
	public static final String DB2_GET_SEQUENCE_OF_COLUMNS1 = "select Column_name,DATA_TYPE,DATA_PRECISION,CHAR_LENGTH from USER_TAB_COLUMNS where table_name='{0}' order by COLUMN_ID";
	public static final String DB2_GET_SEQUENCE_OF_COLUMNS2 = "select Column_name,DATA_TYPE,DATA_PRECISION,CHAR_LENGTH from USER_TAB_COLUMNS where table_name='{0}'  and column_name != '{1}' order by COLUMN_ID";
	public static final String DB2_GET_TABLES_BY_COLUMNS = "select Column_name,DATA_TYPE,DATA_PRECISION,CHAR_LENGTH,table_name from USER_TAB_COLUMNS where column_name in ";
	...
	public static final String DB2_GET_NOT_NULL_CONSTRAINTS_OF_TABLES = "select column_name,data_type,DATA_PRECISION,CHAR_LENGTH,table_name,nullable from USER_TAB_COLUMNS where table_name='{0}' and nullable='N' and Column_name not in ( select column_name from USER_CONS_COLUMNS where table_name='{0}') ";
	public static final String DB2_GET_COLUMNS_OF_TABS_APART_CONSTRAINTS = "select Column_name,DATA_TYPE,DATA_PRECISION,CHAR_LENGTH From USER_TAB_COLS where column_name not in(select column_name From USER_CONS_COLUMNS where TABLE_NAME ='{0}') and table_name='{0}'";
}
