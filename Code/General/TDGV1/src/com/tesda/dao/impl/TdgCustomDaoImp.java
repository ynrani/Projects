					...
					for (String tbName : listFinalSequence) {
						...
						if (!mapPks.containsKey(tbName) && !listMasterTabs.contains(tbName)
							...
							for(Map.Entry<String, Object> mapEntry : mapTempConditions.entrySet()){
								if(String.valueOf(mapEntry.getValue()).contains(TdgCentralConstant.TDG_DB_SEQ) && !mapUkNames.containsKey(mapEntry.getKey()) && !mapNNkNames.containsKey(mapEntry.getKey())){
                           ...
                           // List<Object[]> listParameters = null;
                            for(long i2 = 0;i2 < iLoop;i2++){
    							...
    							logger.info(strClassName + strMethodName
    									...
    									logger.info("Values passed are : "+listParameters);
    									throw e;
    								}
    							
    							}else if(TdgCentralConstant.SCHEMA_GENERATION_TYPE_XLS.equals(testDataGenenarteDTO.getGenerationType())){
    								try {
    									...
    									//following for Enterprise version
											if (i2 == 0) {
												ExcelUtils.doGenerateExcelXLSFile(lstObject,
														...
														mapPositions, null, false);
											} else {
												ExcelUtils.doGenerateExcelXLSFile(lstObject,
														strFilename + ".xls", mapPositions.size(),
														mapPositions, null, true);
											}
    								...
    								try {
    									...
    									if(lstSequenceColsAndRequired != null && !lstSequenceColsAndRequired.isEmpty())
    									cach = new StringBuffer(CSVGenerator.getCSVForObjects(mapPositions, iRequiredRecordCount, listParameters,lstSequenceColsAndRequired));
    									//follwing Enterprise version
    									else
    									cach = new StringBuffer(CSVGenerator.getCSVForList(lstObject));
    									if(cach != null)
